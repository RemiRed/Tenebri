...
public class CharacterScript : NetworkBehaviour
{
	...
	bool hasMoved = false;	//Used to adress a bug when loading the players
...
//			Cursor.visible = false;						//<-- Remember to impelment this everywhere we need to fix with the cursor
    ...
    void Update()
    {
        if (isLocalPlayer)
        {
			//Used to address a bug when loading the players
            if (Input.anyKeyDown) hasMoved = true;
            if (!hasMoved) return;
	...
	void FixedUpdate()
	{
		...
		{
			if (!hasMoved)  return; //Adresses a bug when loading the players
