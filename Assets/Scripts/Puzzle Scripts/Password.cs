...
public class Password : MonoBehaviour
{
    ...
    int nextID = 1,                                     //The ID of the next correct button
        passwordLength = 1,                             //The length of the password
    ...
    public void CheckPassword(int _ID, PlayerCommands playerCmd)
    {
        ...
        if (!solved)
        {
            ...
            { //if wrong button was pressed
                ...
                if (_ID == 1)
                {   //If first password button is pressed, password input is restarted to first password button

                    nextID = 2;

                }
                else if (_ID != nextID - 1)
                {   //Allows the same button to be pressed multiple times without resetting the password
